...
extern "C" {
  ...
  static jboolean is_completed_event = JNI_FALSE;
  ...
  static void
  check_invalid_thread(JNIEnv* jni, char* msg, int err) {
  ...
  Java_UnsupportedTest_isCompleted(JNIEnv *env, jobject this) {
    return is_completed_event;    
  }

  static void
  testUnsupportedMethods(jvmtiEnv *jvmti, JNIEnv *jni, jthread vthread) {
    jvmtiError err;
    ...
    printf("Testing unsupported methods\n");

    err = (*jvmti)->SuspendThread(jvmti, vthread);
    check_invalid_thread(jni,"SuspendThread", err);
    
    err = (*jvmti)->ResumeThread(jvmti, vthread);
    check_invalid_thread(jni,"ResumeThread", err);
    
    err = (*jvmti)->StopThread(jvmti, vthread, vthread);
    check_invalid_thread(jni,"StopThread", err);
    
    err = (*jvmti)->InterruptThread(jvmti, vthread);
    check_invalid_thread(jni,"InterruptThread", err);
    
    err = (*jvmti)->PopFrame(jvmti, vthread);
    check_invalid_thread(jni,"PopFrame", err);
    
    err = (*jvmti)->ForceEarlyReturnVoid(jvmti, vthread);
    check_invalid_thread(jni,"ForceEarlyReturnVoid", err);
    
    err = (*jvmti)->GetThreadLocalStorage(jvmti, vthread, &data);
    check_invalid_thread(jni,"GetThreadLocalStorage", err);
    
    err = (*jvmti)->SetThreadLocalStorage(jvmti, vthread, &data);
    check_invalid_thread(jni,"SetThreadLocalStorage", err);        
    
    err = (*jvmti)->GetThreadCpuTime(jvmti, vthread, &nanos);
    check_invalid_thread(jni,"GetThreadCpuTime", err);
    
  }

  JNIEXPORT jboolean JNICALL
  Java_UnsupportedTest_testExternal(JNIEnv *jni, jobject this) {
    ...
    for (int thread_idx = 0; thread_idx < (int) threads_count; thread_idx++) {
      ...
      if (vthread != NULL) {
        ...
        fflush(0);
      //  lock();
        testUnsupportedMethods(jvmti, jni, vthread);
       // unlock();
      }      
      err = (*jvmti)->ResumeThread(jvmti, thread);      
      check(jni, "ResumeThread", err);
 ...
 //   unlock();
  ...
  extern JNIEXPORT jint JNICALL Agent_OnLoad(JavaVM *jvm, char *options,
    ...
    (*jvmti)->CreateRawMonitor(jvmti, "Events Monitor", &monitor);
